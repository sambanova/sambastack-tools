...
**Total Tests:** 220 automated across 14 suites + a legacy manual test plan
...
- [Automated Tests by Suite](#automated-tests-by-suite)
  ...
  - [Resource Names (Kubernetes `metadata.name` validation)](#resource-names-kubernetes-metadataname-validation)
...
**File:** [model-selection.test.tsx](model-selection.test.tsx) · **Component:** `ModelSelection` (formerly `BundleForm`) · **Tests:** 23
...
| blocks Validate on a bundle name kubectl would reject, naming the offending character | Typing `example_bundle` into Bundle Name flags the `_` in the field and disables Validate/Save, so an RFC-1123-invalid `metadata.name` can no longer reach `kubectl apply`; correcting it to `example-bundle` re-enables them |
| keeps step 4 and its buttons on screen while a multi-arch model is still waiting on its Architecture pick | With a multi-arch model unresolved, Step 4 still renders, names the missing architecture, keeps Validate/Save, and disables Create Deployment; picking the arch generates the YAML and clears the prompt |
| keeps the single-model action bar on screen (Create Deployment disabled) until the model resolves | A single multi-arch model keeps "Advanced Settings"/"Create Deployment" visible with Create Deployment disabled and a "Finish selecting the architecture" prompt; picking the arch enables it |
...
**File:** [model-deployment.test.tsx](model-deployment.test.tsx) · **Component:** `ModelDeploymentManager` (formerly `BundleDeploymentManager`) · **Tests:** 27 (6 status logic + 7 probe-failure logic + 14 integration)
...
| always offers "Ignore EOS" (even without prompt_caching) and injects/removes ENABLE_IGNORE_EOS | The "Ignore EOS" checkbox is always shown (unlike prompt caching); checking injects `engineConfig.env_vars.ENABLE_IGNORE_EOS: "true"`, unchecking removes it (and the emptied `env_vars`) while keeping `startupTimeout` |
| toggles "Ignore EOS" independently of prompt caching (both can coexist in env_vars) | Enabling both puts `ENABLE_IGNORE_EOS` and `ENABLE_KV_CACHE_MANAGER` in `env_vars`; disabling prompt caching leaves the ignore-EOS var intact |
...
| `resolveWildcardTiers` | 4 | Inverse of `collapseTiersToWildcard`: expands a tier's `'*'` to the profile default's batch sizes; leaves explicit lists and wildcards with no matching default; preserves `is_default`; round-trips with the collapse |
...
### Resource Names (Kubernetes `metadata.name` validation)

**File:** [resource-names.test.ts](resource-names.test.ts) · **Module:** `app/utils/resource-names.ts` · **Tests:** 6

Pure validation of the names the user types, so an illegal `metadata.name` is caught in the field
instead of coming back as a `kubectl apply` failure.

| Test | Description |
|------|-------------|
| accepts lowercase RFC 1123 subdomains | Legal names (`bundle1`, `example-bundle`, `b-llama-3p3-70b.v2`, …) pass |
| names the illegal characters, so an underscore is caught in the field and not by kubectl | The message lists each offending character once in first-seen order (`_`, uppercase, space) |
| rejects names that do not start and end with an alphanumeric | Leading/trailing `-` or `.` is reported as a start/end violation, not a charset one |
| requires a name and caps it at the API server limit | Empty is required; 253 chars passes, 254 reports the limit |
| stays quiet while a deployment named after the bundle still fits | A 33-char bundle name (`md-` + 33 = the 36-char pod-name threshold) produces no warning |
| warns once one more character would push the derived pod names over the limit | At 34 chars the warning says pod names would be truncated + hashed and gives the safe length |
