...
export async function GET() {
  try {
    ...
    // Handle versions with non-numerical suffixes (e.g., "sambastack-0.3.510-sharadv-pef-cr-ckpt-sharing-tmp" -> "0.3.510")
    const chartField = sambastackRelease.chart;
    const fullVersion = chartField.replace('sambastack-', '');

    // Extract only the numerical part (major.minor.patch)
    const versionMatch = fullVersion.match(/^(\d+\.\d+\.\d+)/);
    const version = versionMatch ? versionMatch[1] : fullVersion;
    const hasNonNumericalSuffix = fullVersion !== version;
