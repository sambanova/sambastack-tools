...
The example experiments (`codegen_example`, `sqlquerytool_example`, `scicode_example`, `spider_example`), their datasets, the scorers they reference (`codegen_judge`, `sqlquerytool_judge`), and one set of result CSVs are committed so the repo is runnable out of the box. `providers.json` and `test_data.h5` are gitignored — the former is autogenerated and holds secrets, the latter is the large SciCode reference file you download yourself (see [SciCode example](#scicode-example-executing-model-code) below). The SciCode and Spider examples additionally need large datasets you download separately before they can actually *run* (see their sections below).
